...
class CyclicalController extends Controller
{
    ...
    {
        $years = CertificatesView::selectRaw('SUBSTR(`validity_date`,1,4) as `year`')->groupBy('year')->orderBy('year')->havingRaw('COUNT(*) > ?', [100])->get();
    ...
    public function store(Request $request)
    {
        ...
        foreach ($request->year as $year) {
            foreach ($request->type as $type) {
                foreach ($request->position as $position) {
                    ...
                    $months = CertificatesView::selectRaw('SUBSTR(`validity_date`,1,7) as `month`')->where('unit2', $position)->where('unit4', $type)->whereRaw("SUBSTR(`validity_date`,1,4) = $year")->groupBy('month', 'unit4', 'unit2')->orderBy('month')->get();
                    foreach ($months as $month) {
                        $certificates = CertificatesView::where('unit2', $position)->where('unit4', $type)->whereRaw("SUBSTR(`validity_date`,1,7) = '$month->month'")->orderBy('order')->get();
                        foreach ($certificates as $certificate) {
                            ...
                            if ($i % 25 == 1) {
                                ...
                                $sheet->setCellValueByColumnAndRow(1, $i1, "编号:$position-$type-$month->month");
                                //第三行
                                $styleArray = [
                                    'alignment' => [
                                        'horizontal' => 'center',
                                ...
                                if (is_file("storage/sign/$type$position.png")) {
                                    $drawing = new Drawing();
                                    $drawing->setPath("storage/sign/$type$position.png");
                                    $drawing->setHeight(50);
                    ...
                    if ($i > 1) {
                        ...
                        $writer->save($path . "/周检通知单/周检通知单$position$type$year.xlsx");
                    }
                }
            }
        }
        $zip = new ZipArchive();
        if ($zip->open($path . '/周检通知单' . date('Y-m-d') . '.zip', ZipArchive::CREATE) == TRUE) {
        ...
        header("Content-Length: " . filesize($path . '/周检通知单' . date('Y-m-d') . '.zip'));
        header("Content-Disposition: attachment; filename=周检通知单" . date('Y-m-d') . ".zip");
        readfile($path . '/周检通知单' . date('Y-m-d') . '.zip');
        @ob_end_clean();
        if (file_exists($path . '/周检通知单' . date('Y-m-d') . '.zip')) {
            unlink($path . '/周检通知单' . date('Y-m-d') . '.zip');
        }
    }
}
