...
class Role extends Model
{
    ...
    public function getList($request)
    {
        ...
        $positions = Position::where('level', 3)->orderBy('sort')->get();
        ...
        return DataTables::of($query)
            ->addColumn('users', function ($data) {
                return $data->users->pluck('username')->map(function ($user) {
                    return '<span class="tag btn-sm tag-secondary">' . e($user) . '</span>';
                })->implode(' ');
            })
                    ...
                    if ($ids->contains($permission->id)) {
                        ...
                        $html .= $permission->childs->map(function ($permission) use ($ids) {
                            $html = $ids->contains($permission->id)
                                ? '<span class="tag btn-sm tag-outline-primary text-secondary">' . e($permission->description) . '</span> '
                                : '<span class="tag btn-sm tag-outline-secondary">' . e($permission->description) . '</span> ';
                            $html .= $permission->childs->map(function ($permission) use ($ids) {
            ...
            ->addColumn('position', function ($data) use ($positions) {
                ...
                return $positions->map(function ($position) use ($ids) {
                    if ($ids->contains($position->id)) {
                        ...
                        $html .= $position->childs->map(function ($position) use ($ids) {
                            $html = $ids->contains($position->id)
                                ? '<span class="tag btn-sm tag-outline-primary text-secondary">' . e($position->name) . '</span> '
                                : '<span class="tag btn-sm tag-outline-secondary">' . e($position->name) . '</span> ';
                            $html .= $position->childs->map(function ($position) use ($ids) {
