...
@section('content_form')
    ...
    @foreach($certificates as $certificate)
        <div class="row">
            ...
            @include('template.select-btn',['tmp_name'=>'group['.$certificate['json']->id.'][factory_id]','tmp_label'=>'<span class="factory_pdf">'.$certificate['factory'].'</span>','tmp_class'=>'factory_add','tmp_menu'=>'factory','tmp_items'=>$certificate['info']?$certificate['factories']:null,'tmp_value'=>'id','tmp_selected'=>$certificate['info']?$certificate['factory_id']:'','tmp_validate'=>'id','tmp_field'=>'name','tmp_select_class'=>$certificate['info']?'':'is-invalid','tmp_show'=>$certificate['info'],'tmp_state'=>$certificate['info']?'':'disabled','tmp_id'=>$certificate['info']?$certificate['tool_id'].'&name='.$certificate['factory']:'','tmp_reload'=>'group['.$certificate['json']->id.'][tool_id]','tmp_col'=>'4'])
            @include('template.select-btn',['tmp_name'=>'group['.$certificate['json']->id.'][number_id]','tmp_label'=>'<span class="number_pdf">'.($certificate['json']->ccbh=='/'?'':($certificate['json']->ccbh??'')).($certificate['json']->sbbh=='/'?'':($certificate['json']->sbbh??'')).'</span>','tmp_class'=>'number_add','tmp_menu'=>'number','tmp_items'=>$certificate['info']?$certificate['numbers']:null,'tmp_value'=>'id','tmp_selected'=>$certificate['info']?$certificate['number_id']:'','tmp_validate'=>'id','tmp_field'=>'name','tmp_select_class'=>$certificate['info']?'':'is-invalid','tmp_show'=>$certificate['info'],'tmp_state'=>$certificate['info']?'':'disabled','tmp_id'=>$certificate['info']?$certificate['factory_id'].'&name='.($certificate['json']->ccbh=='/'?'':($certificate['json']->ccbh??'')).($certificate['json']->sbbh=='/'?'':($certificate['json']->sbbh??'')):'','tmp_reload'=>'group['.$certificate['json']->id.'][factory_id]','tmp_col'=>'4'])
