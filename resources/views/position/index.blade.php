    ...
    <table id="unsorted-table" class="table table-hover mb-0">
        ...
        @foreach($positions->whereIn('level',$type_id==0?[1,2]:[3,4,5]) as $position)
            @if($type_id==0||in_array($type_id,explode(',',$position->str)))
                <tr @if($position->count==0&&$position->sign==0) class="bg-error" @endif>
                    @switch($position->level)
                        ...
                        <td>@if(isset($position->count))<span class="btn btn-outline-secondary btn-sm ripple">{{$position->count}}</span>@endif</td>
                        @break
                        @case(2)
                        ...
                        <td>@if(isset($position->count))<a class="btn btn-outline-secondary btn-sm ripple" href="?type_id={{$position->id}}">{{$position->count}}</a>@endif</td>
                        @break
                        @case(3)
                        <td><span class="btn btn-warning btn-sm ripple" onclick="m_show('position',{{$position->id}})">{{$position->name1}}</span></td>
                        ...
                        <td>@if(isset($position->count))<span class="btn @if($position->sign==0) btn-outline-secondary @else btn-outline-danger @endif btn-sm ripple" onclick="sign({{$position->id}})">{{$position->count}}</span>@endif</td>
                        @break
                        @case(4)
                        <td>{{$position->name2}}</td>
                        <td><span class="btn btn-success btn-sm ripple" onclick="m_show('position',{{$position->id}})">{{$position->name1}}</span></td>
                    ...
                    <td class="td-actions d-none d-xl-table-cell">
                        ...
                        <a onclick="m_delete('position',{{$position->id}})"><i class="la la-close delete"></i></a>
                    </td>
                </tr>
            @endif
        @endforeach
